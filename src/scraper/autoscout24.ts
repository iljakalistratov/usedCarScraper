...
export async function scrapeAutoscout24() {
        ...
        const data = await page.evaluate(() => {
            ...
            Array.from(carList).forEach((car) => {
              const title1 = (car.querySelector(
                'a.ListItem_title__znV2I h2'
              ) as HTMLElement)?.innerText;
              const title2 = (car.querySelector(
                'span.ListItem_version__jNjur'
              ) as HTMLElement)?.innerText;
              ...
              const link = (car.querySelector(
                'a.ListItem_title__znV2I'
              ) as HTMLElement)?.getAttribute('href');
              const km = (car.querySelectorAll(
                '.VehicleDetailTable_item__koEV4'
              )?.[0] as HTMLElement)?.innerText;
              const year = (car.querySelectorAll(
                '.VehicleDetailTable_item__koEV4'
              )?.[1] as HTMLElement)?.innerText;
              const imgSrc = (car.querySelector('.NewGallery_img__bi92g') as HTMLElement)?.getAttribute('src')?.replace(
                /\/\d+x\d+\.webp$/,
                ''
              );
